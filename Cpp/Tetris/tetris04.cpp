...
//第三版，已经把bug消除，且已经增加高度，降低难度以便多人对决。
//林肯加长
#include <conio.h>
#include <stdlib.h>
#include <Windows.h>
...
void Init()
{
                ...
                textout(handle, dx - 16, dy, wColors, 1, "按任意键开始" );
                 ...
                 int ch = _getch();
                textout(handle, dx - 16, dy, wColors, 1, "MADE BY Flyq  ");
}
...
void GameOver()
{
                 ...
                 for (int i = 0; i < 4; i++)
                {
                                ...
                                textout(handle, 2 + dx, 10 + dy, wColors, 1, "Sorry, Game is over.");
                                Sleep(400);
                                textout(handle, 2 + dx, 10 + dy, wColors, 1, "                    " );
                                Sleep(400);
                }
                exit( EXIT_SUCCESS);
}
